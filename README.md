...
## v39 Updated Fallback Results
- Fallback data updated with completed results through 2026-06-15.
- Added more completed matches for 2026-06-14 and 2026-06-15.
- Added upcoming matches for 2026-06-16 and 2026-06-17.
- If the API fails or CORS blocks the call, Matchday still shows better updated internal data.
- Next step remains: console test for `DYP_API.testFootballData()` and Cloudflare proxy if needed.


## v40 Firebase Connected
- `firebase-config.js` now points to Firebase project `dyp-2df2d`.
- `index.html` loads `firebase-config.js` before `firebase-app.js`.
- Admin email configured as `arvin.bokhoree@gmail.com`.
- Added `FIREBASE_RULES_READY.txt` with rules to paste into Firestore → Rules.
- Storage is intentionally not required, because Firebase Storage requires Blaze. The v1 uses predefined avatars/logos for free.

Next step:
1. Upload v40 files to GitHub root.
2. Paste `FIREBASE_RULES_READY.txt` into Firestore Rules and Publish.
3. Test Google login and Email/Password login.
4. Then create Cloudflare Worker for football-data CORS.


## v41 Firebase Profile Fix
Fixes:
- If a user exists in Firebase Authentication but not in Firestore, the site now automatically creates `wcUsers/{uid}` on login.
- `createProfile()` now handles `auth/email-already-in-use` by logging in and creating/updating the Firestore profile.
- Admin check now correctly uses `ADMIN_EMAILS`.
- Firestore profile fields now include:
  - points
  - badges
  - scorerPredictions
  - scorerBadges
...
## v42 Profile + League Dashboard
New:
- After login, the user sees a real personal dashboard.
- Dashboard shows:
  - DYP Points
  - global rank
  - badges count
  - published cards count
- Global ranking preview shows the top users and highlights the logged-in user.
- Private leagues linked to the logged-in user are listed.
- Selecting a league shows:
  - league header
  - participants
  - private league ranking
  - league cards/activity
- League members now store points, badges and scorerBadges fields.

## v43 Publish Countdown
- Added countdown in Daily Card section.
- Countdown uses the first kickoff of the selected matchday as the publishing deadline.
- Group-stage ET time is parsed as UTC-4.
- The publish button is visually disabled after deadline.
- `publishDailyCard()` also checks the deadline before saving to Firebase.
- If the deadline has passed, the user must move to the next matchday.
