...
async function loadLiveData(){
 ...
 renderMatchday(); updatePublishCountdown();
 renderTeamSelect();
 renderTeamProfile();
 renderAnalytics();
 renderBadges();
...
function renderMatchday(){
 ...
 renderPredictionList(); startPublishCountdown(); refreshAllLiveCards();
}
...
function updatePublishCountdown(){
 const box = document.getElementById("publishCountdownBox");
 const timer = document.getElementById("publishCountdownTimer");
 const text = document.getElementById("publishCountdownText");
 const title = document.getElementById("publishCountdownTitle");
 const publishBtn = document.querySelector('[onclick="DYPAuth.publishDailyCard()"]');
 if(!box || !timer) return;

 const first = getMatchdayDeadline();
 if(!first){
   timer.textContent = "--:--:--";
   if(text) text.textContent = "Nessuna partita disponibile per questa giornata.";
   return;
 }

 const now = new Date();
 const diff = first.deadline - now;
 const isClosed = diff <= 0;
 const underOneHour = diff > 0 && diff < 60*60*1000;

 box.classList.toggle("closed", isClosed);
 box.classList.toggle("warning", underOneHour);
 timer.textContent = formatCountdown(diff);

 if(title) title.textContent = isClosed ? "Scheda chiusa" : "Tempo per pubblicare";
 if(text) {
   text.innerHTML = isClosed
    ? `La scheda per ${first.matchday} è chiusa. Primo kickoff: ${first.home} vs ${first.away}, ${first.time}. <span class="publishClosedNotice">Puoi preparare la prossima giornata.</span>`
    : `La scheda chiude al primo kickoff: ${first.home} vs ${first.away}, ${first.time}.`;
 }

 if(publishBtn){
   publishBtn.disabled = isClosed;
   publishBtn.classList.toggle("disabled", isClosed);
   publishBtn.textContent = isClosed ? "Card closed" : "Publish daily card";
 }
}

function startPublishCountdown(){
 if(countdownInterval) clearInterval(countdownInterval);
 updatePublishCountdown();
 countdownInterval = setInterval(updatePublishCountdown, 1000);
}
